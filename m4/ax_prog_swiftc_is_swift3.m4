...
EOF
        if AC_TRY_COMMAND($SWIFTC $SWIFTCFLAGS -emit-executable -o Test Test.swift) >/dev/null 2>&1; then
            if AC_TRY_COMMAND(./Test one two three) >Test.out 2>&1; then
                test_out=`cat Test.out`
                if test "x$test_out" = ['x["./Test", "one", "two", "three"]']; then
                    ac_cv_prog_swiftc_is_swift3=yes
                else
                    echo "configure: incorrect output was: $test_out" >&AC_FD_CC
                    ac_cv_prog_swiftc_is_swift3=no
                fi
            else
                echo "configure: failed ./Test execution gave:" >&AC_FD_CC
                cat Test.out >&AC_FD_CC
                ac_cv_prog_swiftc_is_swift3=no
            fi
        else
            echo "configure: failed program was:" >&AC_FD_CC
            cat Test.swift >&AC_FD_CC
            ac_cv_prog_swiftc_is_swift3=no
        fi
